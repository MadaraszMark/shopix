...
public class OrderController {
    ...
    @PostMapping
    @Operation(
        summary = "Rendelés létrehozása az aktuális kosárból, kötelező szállítási címmel",
        description = "A szállítási cím (shippingAddressId) kötelező. Ha billingAddressId nincs megadva, a számlázási cím a szállítási cím lesz."
    )
    @ApiResponses({
        ...
        @ApiResponse(responseCode = "400", description = "Hiányzó/érvénytelen címadat vagy üres kosár"),
        @ApiResponse(responseCode = "404", description = "Felhasználó vagy kosár nem található")
    })
    public OrderResponse createFromCart(@AuthenticationPrincipal String email,@Valid @RequestBody CreateOrderRequest request
    ) {
    ...
    @ApiResponses({ @ApiResponse(responseCode = "200", description = "Rendelések listája") })
    public Page<OrderResponse> listMyOrders(@AuthenticationPrincipal String email,@Parameter(description = "Lapozás és rendezés paraméterei") Pageable pageable) {
        Long userId = currentUser.requireId(email);
